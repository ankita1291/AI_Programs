...
class EightPuzzle:
    ...
    def solve(self, h):
        ...
        while len(openl) > 0:
            ...
            if (is_solved(x)):
                if len(closedl) > 0:
                    print(x._generate_solution_path([]))
...
def main():
    ...
    #     for i in path: 
    #         print (i)
    #     print ("count of flipped tiles ", count, "states")
    #     tc =tc+count
    # print("Average with flipped ", tc/5)
    for i in range(5):
        path, count = p.solve(h_linear)
        path.reverse()
        for i in path: 
            print (i)
        print ("linear conflict exploring", count, "states")
        tc = tc + count
    print("Average linear conflict", tc/5)
